...
fun Button(
    ...
    // COUI press feedback: the whole button scales down (~0.92 for large buttons) and the fill
    // darkens to 80% brightness while pressed (COUIButton scaleEnable + brightness 0.8).
    ...
    val pressedContainerColor by animateColorAsState(
        targetValue = if (enabled && isPressed) {
            // brightness 0.8 == composite 20% black over the fill.
            Color.Black.copy(alpha = 1f - ButtonDefaults.PressedBrightness).compositeOver(containerColor)
        } else {
            containerColor
        },
    ...
    CompositionLocalProvider(LocalContentColor provides contentColor) {
        Box(
            modifier = modifier
                ...
                .squircleSurface(color = pressedContainerColor, cornerRadius = cornerRadius)
                .clickable(
...
object ButtonDefaults {
    ...
    /**
     * The scale a button shrinks to while pressed (COUI a2.b large-control press scale 0.92).
     */
    val PressedScale = 0.92f

    /**
     * The fill brightness multiplier while pressed (COUIButton brightness 0.8).
     */
