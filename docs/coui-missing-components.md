...
| `COUIButtonBarLayout` | 对话框按钮栏自适应：按钮过宽自动纵排、等宽策略（我们 Dialog 目前仅调用方手排） | 中 | `S`: `buttonBar\COUIButtonBarLayout.java`、`button\SingleButtonWrap.java`、`button\SimpleButtonGroupCtrl.java`；layout `coui_bottom_alert_dialog_button_panel.xml`、`coui_center_alert_dialog_button_panel.xml` | `layout/DialogContentLayout.kt` 增强 |
...
| `COUILottieLoadingView` | Lottie 加载动画视图 | 低 | `S`: `progressbar\COUILottieLoadingView.java` | 不建议做（依赖 Lottie，平台受限） |
| `COUICompProgressIndicator` | 加载指示器 + 文案组合件 | 低 | `S`: `progressbar\COUICompProgressIndicator.java` | InfiniteProgressIndicator + Text 即可 |
...
| ProgressIndicator | 大体完备 | 定值/不定值/large 档齐（本轮补 large）。📝 COUICircularProgressBar 另有 **error / pause 图标态**（`coui_circular_progress_error_*` / `pause_icon_*` dimens），下载场景用，随 InstallLoadProgress 一并评估 |
