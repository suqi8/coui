...
| `COUIButtonBarLayout` | 对话框按钮栏自适应：按钮过宽自动纵排、等宽策略（我们 Dialog 目前仅调用方手排）。**横排一档的度量已对齐**（58dp 最小高、按钮 24dp/12dp/22dp 内边距、1dp 分割线内缩 17dp/21dp，见 `DialogDefaults.ButtonBar*`）；**仍缺自动纵排**：`onMeasure` 用 `needSetButVertical(min(coui_dialog_max_width=392dp, measuredWidth))` 判定，可用宽度按钮数均分后减去 `2 * coui_alert_dialog_button_horizontal_padding`，任一按钮文本实测更宽即整栏翻成纵排。纵排另有一套度量：`coui_alert_dialog_vertical_button_min_height` 52dp（最底部按钮加 `coui_center_alert_dialog_vertical_button_paddingbottom_vertical_extra` 12dp = 64dp）、垂直内边距 `coui_bottom_alert_dialog_vertical_button_padding_vertical_new` 14dp、栏 topMargin `coui_bottom_alert_dialog_buttonbar_margintop` 16dp。我们目前长文案只会被挤压/截断 | 中 | `S`: `buttonBar\COUIButtonBarLayout.java`、`button\SingleButtonWrap.java`、`button\SimpleButtonGroupCtrl.java`；layout `coui_bottom_alert_dialog_button_panel.xml`、`coui_center_alert_dialog_button_panel.xml` | `layout/DialogContentLayout.kt` 增强 |
...
| `dialog\COUIAlertDialogBuilder` | `overlay/OverlayDialog.kt` + `window/WindowDialog.kt` | 缺 security / rotating 变体（见高优先级）；按钮栏纵横自适应见 `COUIButtonBarLayout` |
