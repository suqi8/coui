...
`RadioButton` is a basic selection component in Miuix, supporting two states: selected and unselected. When selected, it displays an animated checkmark indicator matching the miuix-classic SingleChoicePreference style. When unselected, no indicator is shown. It is suitable for single selection scenarios where only one option can be chosen from a group.
